...
/**
 * RecalculatingGrid-specific permutation runner for parallel parameter sweep simulations.
 *
 * Runs ~1,600 RecalculatingGrid simulations with different parameter combinations using Virtual Threads.
 *
 ...
 *
 * RecalculatingGrid parameter ranges (default):
 ...
 */
@Slf4j
public class RecalculatingGridPermutationRunner {
    ...
    private static void runPermutations(String candleFileName, boolean saveToMongo) throws Exception {
        log.info("Starting RecalculatingGrid permutation run");
        ...
        RecalculatingGridParameterPermutation permutation = RecalculatingGridParameterPermutation.defaultPermutation();
        List<RecalculatingGridConfig> configurations = permutation.generateConfigurations();
        ...
        for (int i = 0; i < configurations.size(); i++) {
            RecalculatingGridConfig config = configurations.get(i);
        ...
        if (saveToMongo) {
            ...
            log.info("MongoDB persistence enabled (collection: RecalculatingGrid-{})", tradingPair);
        }
        ...
        for (int i = 0; i < topTenWithConfigs.size(); i++) {
            ...
            RecalculatingGridConfig config = (RecalculatingGridConfig) taskResult.getAlgorithmConfig();
        ...
        if (saveToMongo) {
            ...
            for (int i = 0; i < topTenWithConfigs.size() && i < 10; i++) {
                ...
                List<com.tradingbot.persistence.entities.SimulationResultDocument> allDocs =
                    repository.findAll("RecalculatingGrid", tradingPair);
        ...
        if (!topTenWithConfigs.isEmpty()) {
            RecalculatingGridConfig bestConfig = (RecalculatingGridConfig) topTenWithConfigs.get(0).getAlgorithmConfig();
            System.out.printf("%nBest Config: %s%n", bestConfig.getConfigId());
        }
        System.out.println("=======================================");

        log.info("RecalculatingGrid permutation run completed successfully");
    }
}
